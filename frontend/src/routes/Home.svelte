<script>
  ...
  let size = 10
  let kw = ''
  // let page = 0   # store 처리로 변수 삭제 - store 에서 page 변수 정의
  
  // 변수 앞에 $: 기호를 붙이면 해당 변수는 반응형 변수가 된다. 
  // 즉, total 변수의 값이 API 호출로 인해 그 값이 변하면 
  // total_page 변수의 값도 실시간으로 재 계산된다는 의미
  ...
  <div class="row my-3">
    ...
    <div class="col-6">
        <div class="input-group">
            <input type="text" class="form-control" bind:value="{kw}" on:keydown={(e) => {if (e.key === 'Enter') search_question()}}>
            <button class="btn btn-outline-secondary" on:click={() => {search_question()}}>
                찾기
            </button>
        </div>
    </div>
